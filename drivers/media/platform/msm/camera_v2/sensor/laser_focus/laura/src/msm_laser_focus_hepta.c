...
static void start_continuous_measure_work(void)
{
	...
	if(!measure_thread_run)
	{
		...
		LOG_Handler(LOG_CDBG,"kick off measure thread func for continuous-measurement mode\n");
		schedule_delayed_work(&measure_work, 0);
		measure_thread_run = true;
        //wait the laser sensor read the first distance for max 1s, if the laser sensor
        //have some error because hardware error, we can skip it, Nor ,it will block the phone bring up
        //not to wait the laser update, because sometime it will block the usr call, which will block the camera bring up for a long time
        #if 0
		while((!measure_cached_range_updated)&&index<=10)
		{
		    index++;
			msleep(100);
		}
		#endif
		//not to wait the laser update, because sometime it will block the usr call, which will block the camera bring up.
...
void continuous_measure_thread_func(struct work_struct *work)
{
    ...
    LOG_Handler(LOG_ERR,"%s(), the enable lunch the thread, fist let us power up, laser_on_pending=%d\n",__func__,laura_t->laser_on_pending);
	if(laura_t->laser_on_pending){
    power_up(laura_t);
	dev_init(laura_t);
	msleep(500);
    if( laura_t->pending_value== MSM_LASER_FOCUS_DEVICE_APPLY_CALIBRATION)
    Laura_device_power_up_init_interface(laura_t, DO_CAL, &calibration_flag, NO_MEASURE);
    else
    Laura_device_power_up_init_interface(laura_t, NO_CAL, &calibration_flag, NO_MEASURE);	
	
    load_calibration_data = (laura_t->pending_value == MSM_LASER_FOCUS_DEVICE_APPLY_CALIBRATION?true:false);
	laura_t->laser_on_pending=0;
	}
	...
	if(continuous_measure)
	{
		do{
            ...
            LOG_Handler(LOG_ERR,"%s(), Start to read the laser distance, the function will got to a loop uttil a exit flag or read cci error\n",__func__);
			...
			if(measure_thread_run)
			{
				...
				while(measure_thread_run)
				{
					if(laura_t->device_state == MSM_LASER_FOCUS_DEVICE_OFF){
						LOG_Handler(LOG_ERR,"device was not opened, disable laser ahead\n");
						break;
					}
					msleep(1);
				}
...
int Laser_Enable(enum msm_laser_focus_atd_device_trun_on_type val){
		...
		LOG_Handler(LOG_ERR, "%s: client %d, one client enable the devices, set laser pending, we will actualy power up the devices when the read thread start\n ", __func__,client);
        laura_t->laser_on_pending=1;
        laura_t->pending_value=val;
		...
		if(continuous_measure)
		{
		LOG_Handler(LOG_ERR, "%s: client %d, start the measure thread\n ", __func__,client);
		start_continuous_measure_work();
		}

		mutex_ctrl(laura_t, MUTEX_UNLOCK);
		LOG_Handler(LOG_ERR, "%s Init Device (%d) return enable always success\n", __func__, laura_t->device_state);
		//return rc;
		return 0;//always return success , because we want start_continuous_measure_work to recovery the status 
}
void HPTG_Customize(struct HPTG_settings* Setting){
       ...
       if(Sysfs_read_word_seq("/persist/HPTG_Settings.txt",inputData,NUMBER_OF_SETTINGS)>=0){
...
*/
	...
	LOG_Handler(LOG_ERR, "%s: command (%d) done\n",__func__,val);
	return ret_len;
...
static void test_log_time(int count)
{
	...
	for(i=0;i<count;i++)
	{
		LOG_Handler(LOG_ERR,"czw, test log %dth log!\n",i);
	}
	
	
	do_gettimeofday(&t2);
	
	LOG_Handler(LOG_ERR,"czw, print %d logs cost %lu us\n",count,diff_time_us(&t2,&t1));
}
...
static int ATD_Laura_device_get_range_read(struct seq_file *buf, void *v)
{
...
#endif
	LOG_Handler(LOG_ERR, "%s: Enter laura_t->laser_on_pendin=%d, continuous_measure=%d\n", __func__,laura_t->laser_on_pending,continuous_measure);

    if(laura_t->laser_on_pending)
    {
      Range = 0;
	  seq_printf(buf, "%d\n", Range);
      LOG_Handler(LOG_ERR,"%s: the laser power up  ing ....\n",__func__);
	  return 0;
    }
	...
	if(laser_focus_enforce_ctrl != 0){
		...
		LOG_Handler(LOG_ERR, "%s: laser_focus_enforce_ctrl (%d) \n", __func__, laser_focus_enforce_ctrl);
		mutex_ctrl(laura_t, MUTEX_UNLOCK);
		return 0;
	}
...
static const struct file_operations user_version_debug = {
	.owner = THIS_MODULE,
	.open = ATD_Laura_I2C_status_check_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
if(Laser_Product == PRODUCT_OLIVIA){
	LOG_Handler(LOG_ERR,"czw, OLIVIA create calibration fops");
	proc(DEVICE_SET_CALIBRATION, 0666, &ATD_olivia_calibration_fops);
	proc(DEVICE_GET_CALIBRATION_INPUT_DATA, 0666, &ATD_Olivia_get_calibration_input_data_fops);
}
else{
	proc(DEVICE_SET_CALIBRATION, 0666, &ATD_laser_focus_device_calibration_fops);
	proc(DEVICE_GET_CALIBRATION_INPUT_DATA, 0666, &ATD_Laura_get_calibration_input_data_fops);
}
	...
	proc(DEVICE_LOG_CTRL_FILE, 0666, &laser_focus_log_contorl_fops);

	proc(DEVICE_CALIBRATION_UPDATED, 0666, &need_load_calibration_fops);
	proc(DEVICE_MEASURE_MODE, 0666, &measure_mode_fops);
...
static int match_Olivia(struct platform_device *pdev){
	...
	if (!laura_t) {
		LOG_Handler(LOG_ERR,"%s:%d failed no memory\n", __func__, __LINE__);
		return -ENOMEM;
	}

	HPTG_t = kzalloc(sizeof(struct HPTG_settings),GFP_KERNEL);
	if (!HPTG_t) {
		   LOG_Handler(LOG_ERR,"%s:%d failed  \n", __func__, __LINE__);
		   return -ENOMEM;
	}
...
#endif
		kfree(laura_t);
		LOG_Handler(LOG_ERR,"failed no memory\n");
		return -ENOMEM;
	}
...
#if !LASER_USE_VREG
static int laser_config_gpio(struct msm_laser_focus_ctrl_t *laura_t)
{
    int retval = 0;

    retval = gpio_request(laura_t->pwr_en_gpio, "power enable");
    if (retval < 0) {
        LOG_Handler(LOG_ERR,"Failed to request GPIO:%d, ERRNO:%d",laura_t->pwr_en_gpio, retval);
        return -ENODEV;
    }

       /* Get pinctrl if target uses pinctrl */
       laura_t->laser_pinctrl = devm_pinctrl_get(&(laura_t->pdev->dev));
       if (IS_ERR_OR_NULL(laura_t->laser_pinctrl)) {
               retval = PTR_ERR(laura_t->laser_pinctrl);
               dev_dbg(&laura_t->pdev->dev,
                       "Target does not use pinctrl %d\n", retval);
               goto err_pinctrl_get;
       }

       laura_t->pinctrl_state_active
               = pinctrl_lookup_state(laura_t->laser_pinctrl,
                               "laser_active");
       if (IS_ERR_OR_NULL(laura_t->pinctrl_state_active)) {
               retval = PTR_ERR(laura_t->pinctrl_state_active);
               dev_err(&laura_t->pdev->dev, "Can not lookup active pinstate %d\n", retval);
               goto err_pinctrl_lookup;
       }


       laura_t->pinctrl_state_suspend
               = pinctrl_lookup_state(laura_t->laser_pinctrl,
                       "laser_suspend");
       if (IS_ERR_OR_NULL(laura_t->pinctrl_state_suspend)) {
               retval = PTR_ERR(laura_t->pinctrl_state_suspend);
               dev_err(&laura_t->pdev->dev,"Can not lookup suspend pinstate %d\n", retval);
               goto err_pinctrl_lookup;
       }
...
err_pinctrl_lookup:
       devm_pinctrl_put(laura_t->laser_pinctrl);
err_pinctrl_get:
       laura_t->laser_pinctrl = NULL;
       return retval;
...
static int32_t Olivia_platform_probe(struct platform_device *pdev)
{
	...
	proc("driver/Probe_Start", 0664, &user_version_debug);
	LOG_Handler(LOG_ERR,"czw, Olivia_platform_probe Enter");
/*
	////
...
*/
	...
	if(rc < 0){
		 proc("driver/match_Olivia", 0664, &user_version_debug);
	     LOG_Handler(LOG_CDBG, "%s: match_Olivia faild, proble fail\n", __func__);
		 goto probe_failure;
	...
	if (rc < 0) {
		proc("driver/get_dtsi_data", 0664, &user_version_debug);
	    LOG_Handler(LOG_CDBG, "%s: get_dtsi_data faild, proble fail\n", __func__);
		goto probe_failure;
	}

#if !LASER_USE_VREG

	LOG_Handler(LOG_CDBG, "%s: start to config gpio\n", __func__);
	rc = laser_config_gpio(laura_t);
	if (!rc && laura_t->laser_pinctrl) {

            
	        LOG_Handler(LOG_CDBG, "%s: config gpio successed, start to active use pinctl policy\n", __func__);
			rc = pinctrl_select_state(laura_t->laser_pinctrl,
									laura_t->pinctrl_state_active);
			if (rc < 0) {
					dev_err(&laura_t->pdev->dev,
							"failed to select pin to active state");
			}
			gpio_direction_input(laura_t->pwr_en_gpio);
	}
#endif
	...
	{
	   proc("driver/dev_I2C_status_check", 0664, &user_version_debug);
	   goto probe_failure;
	}

	LOG_Handler(LOG_CDBG, "%s: we have correctly read the laser id, continue creat dev node\n", __func__);
	/* Init mutex */
	...
	if (rc < 0){
		proc("driver/Olivia_misc_register", 0664, &user_version_debug);
		goto probe_failure;
		}
	...
	LOG_Handler(LOG_ERR,"czw, Olivia_platform_probe Success");
	LOG_Handler(LOG_CDBG, "%s: Probe Success\n", __func__);
	proc("driver/Probe_Success", 0664, &user_version_debug);
	return 0;
	
probe_failure:
	LOG_Handler(LOG_ERR,"czw, Olivia_platform_probe failed, rc = %d\n",rc);
